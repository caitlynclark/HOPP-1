...
def plot_h2_results(H2_Results, 
    ...
    if plot_h2:
        ...
        # plt.plot(load, label = 'Load') #[200:300]) #,label="electrolyzer rating")
        plt.legend()
        ...
        plt.tight_layout()
        plt.xlabel('Time (hours)')
        plt.savefig(os.path.join(results_dir,'Electrolyzer Flows_{}_{}.jpg'.format(site_name,atb_year)),bbox_inches='tight') #, format="png", dpi=1200)
        # plt.show()
        plt.close("all")
        plt.clf()
