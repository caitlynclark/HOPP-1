...
class PEM_H2_Clusters:
    ...
    def run(self,input_external_power_kw):
        ...
        if self.include_deg_penalty:
            ...
            V_cell_deg,deg_signal=self.call_degradation_calculations(V_init)
            ...
            V_init=self.cell_design(self.T_C,stack_current)
            V_ignore,deg_signal=self.full_degradation(V_init,self.cluster_status)
    ...
    def full_degradation(self,voltage_signal,operational_status):
        #TODO: add reset if hits end of life degradation limit!
        # voltage_signal = voltage_signal*self.cluster_status
        voltage_signal = voltage_signal*operational_status
        if self.use_uptime_deg:
            V_deg_uptime = self.calc_uptime_degradation(voltage_signal,operational_status)
        else:
            V_deg_uptime=np.zeros(len(voltage_signal))
        if self.use_onoff_deg:
            V_deg_onoff = self.calc_onoff_degradation(operational_status)
        else:
    ...
    def call_degradation_calculations(self,cell_voltage_signal):
        ...
        degraded_voltage_signal,Vdeg_signal=self.full_degradation(cell_voltage_signal,self.cluster_status)
        deg_df_temp=pd.concat([deg_df,self.output_dict['Cumulative Degradation Breakdown']])
        ...
        if stack_died:
            ...
            print('stack died at hour {}'.format(hour_of_death))
            # n_stackrep_per_sim +=1
            # stack_lived_hrs.append(hour_of_death)
            deg_df=pd.concat([deg_df_temp.iloc[0:int(hour_of_death)],self.output_dict['Cumulative Degradation Breakdown']])
            if next_stack_will_die:
                ...
                while next_stack_will_die:
                    ...
                    print('next stack will die at hour {}'.format(hour_of_death))
                    # deg_df=pd.concat([deg_df,self.output_dict['Cumulative Degradation Breakdown']])
                    deg_df=pd.concat([deg_df.iloc[0:int(hour_of_death)],self.output_dict['Cumulative Degradation Breakdown']])
                    stack_lived_hrs.append(hour_of_death)
                    n_stackrep_per_sim +=1
                    loop_counter+=1
        ...
        if n_stackrep_per_sim>0:
            #ADDING
            sim_ending_degradation=Vdeg_final[int(hour_of_death):len(Vdeg_final)]
            #sim_ending_operational_hours = self.cluster_status[int(hour_of_death):len(Vdeg_final)]
            hours_til_next_replacement=(self.d_eol/Vdeg_final[-1])*(len(sim_ending_degradation))
            estimated_hour_of_future_death = int(hour_of_death) + hours_til_next_replacement
            stack_lived_hrs.append(estimated_hour_of_future_death)
            #####
            stack_replacement_schedule_yrs=self.make_stack_replacement_schedule_new(stack_lived_hrs)
            []
    ...
    def make_stack_replacement_schedule_new(self,stack_lived_hrs):
        replacement_schedule_yrs_temp=np.zeros(self.plant_life_years)
        plant_life_hrs=self.plant_life_years*8760
        #sim_length_hrs=len(self.cluster_status)
        #replacement_schedule_yrs_temp=np.zeros(self.plant_life_years)
        stack_life_length=[int(stack_lived_hrs[0])] + list(np.diff(stack_lived_hrs))
        last_sim_death=stack_lived_hrs[-1]
        num_repeats=np.ceil(plant_life_hrs/last_sim_death)
        years_replace=np.floor(np.array(stack_lived_hrs)/8760)
        for y in years_replace:
            replacement_schedule_yrs_temp[int(y)]=1
        replace_list=list(replacement_schedule_yrs_temp[0:int(y)+1])*int(num_repeats)
        refurbishment_schedule = np.array(replace_list[0:self.plant_life_years])
        return refurbishment_schedule
        # operational_hours=[]
        # last_real_death_hour=0
        # for si,stack_lifes in enumerate(stack_lived_hrs):

        #     if stack_lifes>sim_length_hrs:
        #         last_real_death_hour = int(stack_lived_hrs[si-1])
        #         op_hrs=np.sum(self.cluster_status[last_real_death_hour:sim_length_hrs])
        #     else:

        #         op_hrs=np.sum(self.cluster_status[last_real_death_hour:int(stack_lifes)])
        #         last_real_death_hour=int(stack_lifes)
        #     operational_hours.append(op_hrs)

        

    def make_stack_replacement_schedule(self,stack_lived_hrs):
        #NOTE: unused as of now
        #TODO: check that we're looking at the last little big of life before next death
        #NOTE: this has not been checked for correctness
        #This is probably overcomplicated also
        
        plant_life_hrs=self.plant_life_years*8760
        sim_length_hrs=len(self.cluster_status)
        num_sims_4_life=int(np.ceil(plant_life_hrs/sim_length_hrs))
        replacement_schedule_yrs_temp=np.zeros(self.plant_life_years)
        life_start=0
        ...
        for life_duration_hrs in operational_hours:
            #ISSUE ALSO HERE
            replacement_year=int(np.floor(life_duration_hrs/8760))
            replacement_schedule_yrs_temp[replacement_year]+=1
        
        replace_sched=list(replacement_schedule_yrs_temp[0:replacement_year+1])*num_sims_4_life
    ...
    def check_aliveness(self,deg_signal_init,voltage_signal_noDeg):
        ...
        if deg_signal_init[-1]>self.d_eol:
            ...
            cluster_status_next_life = self.cluster_status[idx_dead:]
            #temp_v_thislife=np.concatenate([np.zeros(idx_dead),voltage_signal_next_life])
            #v_tot_next_life,deg_next_life=self.full_degradation(temp_v_thislife)
            #workaround for length mismatch between self.cluster_status and voltage_signal_next_life
            #
            v_tot_next_life,deg_next_life=self.full_degradation(voltage_signal_next_life,cluster_status_next_life)
    ...
    def calc_uptime_degradation(self,voltage_signal,operational_status):
        #steady_deg_rate = 1.12775521e-09
        
        
        steady_deg_per_hr=self.dt*self.steady_deg_rate*voltage_signal*operational_status#self.cluster_status
        cumulative_Vdeg=np.cumsum(steady_deg_per_hr)
        # self.output_dict['Total Uptime [sec]'] = np.sum(self.cluster_status * self.dt)
        self.output_dict['Total Uptime [sec]'] = np.sum(operational_status * self.dt)
        self.output_dict['Total Uptime Degradation [V]'] = cumulative_Vdeg[-1]

        return steady_deg_per_hr
        
    def calc_onoff_degradation(self,operational_status):
        
        
        # change_stack=np.diff(self.cluster_status)
        change_stack=np.diff(operational_status)
    ...
    def run_grid_connected_workaround(self,power_input_signal,current_signal):
        ...
        V_cell_deg,deg_signal=self.full_degradation(V_init,self.cluster_status)
        nsr_life=self.calc_stack_replacement_info(deg_signal)
