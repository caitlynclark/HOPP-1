...
electrolyzer_size_mw = 50 #must be an integer
number_of_electrolyzer_stacks = 5 #electrolyzer_size_mw must be divisible by this number
...
plot_h2 = False
plot_diesel=True
turbine_name =  'VestasV47_660kW_47' #'VestasV82_1.65MW_82' #'VestasV47_660kW_47' #
h2_model ='Simple'  
# h2_model = 'H2A'
...
results_dir = parent_path + '/examples/H2_Analysis/Nome/results/'
...
generator_type='CAT' #options: 'CAT' or 'WRS' or 'Both'
forced_diesel_sizes=True #if false, uses as many generators as it takes to get to 0 shortfall
force_diesel_cat=True #only used if forced_diesel_sizes is True and generator type is 'CAT' or 'Both'
...
force_diesel_warsilla=False #only used if forced_diesel_sizes is True and generator type is 'WRS' or 'Both'
force_wrs_size_kw= 16000 #num_wrs_gen * 9390 #10000 #only used if generator type is 'WRS' or 'Both'
    ...
    for atb_year in atb_years:
        for i, site_deet in enumerate(site_details.iterrows()):
            ...
            energy_shortfall_hopp, gen, annual_energies, wind_plus_solar_npv, npvs, lcoe =  \
                ...
                kw_continuous, load,
                custom_powercurve,
                electrolyzer_size_mw, grid_connected_hopp=True, wind_om_cost_kw=wind_om_cost_kw)
           ...
           #Check Percentage of time load is met with storage
            ...
            if plot_battery:
                ...
                # plt.plot(final_diesel_shortfall[892:899],"--",label="Shortfall")
                ...
                # plt.subplot(312)
                # plt.plot(combined_pv_wind_storage_diesel_power_production_hopp[892:899],label="Wind+PV+Storage+Diesel")
                ...
                #                           final_diesel_shortfall,total_diesel_power_supplied,
                #                           total_diesel_fuel_used,],
                #                           ['Curtailment', 'Shortfall', 'Wind + PV + Storage', 
                #                           'Wind + PV Generation', 'Load', 'State of Charge', 
                #                           'Battery Used', 'Final Diesel Shortfall', 
                #                           'Total Diesel Power Supplied', 'Total Diesel Fuel Used'])
                # df_outage.to_csv(os.path.join(results_dir, "DIESEL_Hybrid_TotalLoad_Feb_Algona_{}_{}.csv".format(scenario_choice, forced_storage_size_mwh)))
                ...
                plt.subplot(312)
                plt.plot(combined_pv_wind_storage_diesel_power_production_hopp[3206:3254],label="Wind+PV+Storage+Diesel")
                ...
                df_outage = pd.DataFrame([combined_pv_wind_curtailment_hopp[3206:3254], 
                                          ...
                                          final_diesel_shortfall[3206:3254],total_diesel_power_supplied,
                                          total_diesel_fuel_used,],
                                          ['Curtailment', 'Shortfall', 'Wind + PV + Storage', 
                                          'Wind + PV Generation', 'Load', 'State of Charge', 
                                          'Battery Used', 'Final Diesel Shortfall', 
                                          'Total Diesel Power Supplied', 'Total Diesel Fuel Used'])
                df_outage.to_csv(os.path.join(results_dir, "DIESEL_Hybrid_TotalLoad_May_Algona_{}_{}.csv".format(scenario_choice, forced_storage_size_mwh)))
